...
- 首屏口径：时间、搜索框、主体布局先完成；启动遮罩会短暂等待壁纸结果，远程图标、访问统计、状态检测、更新检查不进入首屏等待。
...
- [x] 壁纸加载时序：远程壁纸在首屏绘制后立即加载，并向启动遮罩回报 `load/error` 状态。
- [x] 首屏启动遮罩：短暂显示轻量加载动画，遮住页面组装过程，等待壁纸结果或超时后淡出完整页面。
