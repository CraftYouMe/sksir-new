...
- 本轮快捷入口拖动残影和设置字体修复尚未提交；目标版本为 `2026.08.03.13`。上一轮已完成 Git 提交 `a205219`，并成功上传、验证 `2026.08.03.12` OSS 版本目录，共 106 个资源；本轮尚未触发 OSS 上传、Vercel 推送或网站生产部署。
...
| 2026-08-03 | 完成 Chromium 扩展本地化构建、启动接管、浏览器启动页适配、favicon 本地缓存、首屏搜索框稳定渲染、本地壁纸预加载、搜索聚焦态白色背景/深色图标、搜索进出状态类、首次加载取消过渡优先级、关闭状态帧、背景模糊恢复过渡、收藏中心背景聚焦效果、快捷入口拖动残影和设置选项字体统一修复。当前版本为 `2026.08.03.13`。 |
