...
- 百度搜索建议、输入防抖和键盘上下选择
- `Esc` 可快速关闭搜索、书签和设置层
- 桌面端按 `/` 可直接聚焦搜索；书签、设置和分类支持方向键、`Home/End`、回车与空格操作
- 键盘关闭面板后焦点会回到时间入口，并仅在键盘操作时显示轻量焦点提示
...
- `/` 搜索快捷键只在桌面精细指针环境、且当前不在输入控件或面板内时生效，不占用 `Ctrl/Cmd+K` 等浏览器快捷键。
- 书签标签、设置标签和分类筛选使用现有点击逻辑完成键盘激活，不增加首屏资源；焦点样式使用 `:focus-visible`，不会改变普通鼠标和触摸点击布局。
