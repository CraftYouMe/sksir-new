window.SKSIR_CHANGELOG = {
  "generatedAt": "2026-08-02T08:45:18.654Z",
  "source": "git",
  "entries": [
    {
      "hash": "本次更新",
      "date": "2026-08-02",
      "summary": "解决设置中心首次切换与快捷入口拖动启动显示",
      "details": [
        "设置资源在首屏流程后预热，刷新后首次打开设置切换更顺滑",
        "恢复快捷入口拖动节点的屏幕外定位，避免拖动刚启动时首个入口出现图标残影"
      ],
      "tags": [
        "fix",
        "optimize"
    ...
    },
    {
      ...
      "details": [
        "顶部导航改为真实 Flex 工具栏，移动端 Tab 滚动区域与右侧操作按钮完全分离",
        "解决 Tab hover 下划线偏移，激活标识改为直角短线",
        "统一测速与设置按钮的 32px 尺寸、底色和图标颜色",
        "桌面收藏卡片缩窄为每行 5 个，移动端继续保持每行 2 个"
      ],
      "tags": [
        "fix"
      ]
    },
    {
      ...
      "summary": "统一设置中心控件状态",
      "details": [
        "为设置侧栏、页面标签和按钮补充清晰一致的键盘焦点轮廓",
        "默认搜索引擎卡片改用统一选中态设计令牌，保持设置卡片视觉一致",
        "调整设置操作项圆角，使其与现有控件圆角体系一致"
      ],
      "tags": [
        "improve"
      ]
    },
    {
      "hash": "1fbad0d",
      "date": "2026-08-02",
      "summary": "重写收藏中心一级导航工具栏",
      "details": [
        "一级 Tab 升级为 56px 玻璃导航栏和 40px 完整圆角标签，解决标签贴底截断的观感",
        "激活标签使用轻玻璃选中面、内高光和按钮内部渐变标识，强化主导航层级",
        "测速入口迁移到搜索、设置和关闭按钮所在的右侧工具组，四个入口保持同一基线",
        "移动端保留横向导航能力，测速结果继续通过收藏卡片状态圆点显示"
      ],
      "tags": [
        "fix"
      ]
    },
    {
      "hash": "48a4d0b",
      "date": "2026-08-02",
      "summary": "精修收藏中心顶部导航与卡片留白",
      "details": [
        "搜索、设置和关闭按钮与一级分类 Tab 调整到同一水平基线，移动端继续支持横向滚动",
        "搜索、设置和关闭图标重新绘制，设置入口取消旋转并改为静态调节器图标",
        "搜索展开态加深玻璃底色，避免覆盖导航时文字透叠",
        "quick-alls 上下间距统一，收藏列表留白更加均衡"
      ],
      "tags": [
        "new",
        "improve"
    ...
    },
    {
      ...
      "summary": "改善收藏中心悬浮工具栏",
      "details": [
        "收藏搜索默认收为圆形图标，点击后以 240ms 宽度动画展开并自动聚焦",
        "搜索收起时清空查询并恢复原分类，避免隐藏搜索条件继续过滤收藏",
        "设置、关闭与状态检测入口统一为 32px 图标按钮，减少顶部区域高度",
        "状态检测取消文字按钮，点击后通过每张收藏卡片的状态圆点展示详细结果"
      ],
      "tags": [
        "optimize"
      ]
    },
    {
      "hash": "142f6f5",
      "date": "2026-08-01",
      "summary": "重塑收藏中心轻量导航并统一核心界面",
      "details": [
        "精简收藏中心重复标题，顶部仅保留搜索与操作，让一级标签成为主要导航",
        "放大一级标签并增加横向间距，二级分类继续作为小号筛选标签",
        "降低收藏卡片、图标容器、分类栏和面板边框与背景强度，保持 76px 横向结构",
        "统一正式页面的设计令牌、设置侧栏、表单控件与管理卡片交互，并同步 design-test"
      ],
      "tags": [
        "improve"
      ]
    ...
    },
    {
      ...
      "details": [
        "收藏中心桌面标题提升至 28px，并降低副标题对比度，让面板标题成为第一视觉焦点",
        "缩小搜索框和一级标签高度，收紧标题、分类、筛选与收藏列表之间的连续间距",
        "收藏卡片保持管理卡片视觉并压缩至 72px，添加卡片改为低权重的居中大号加号"
      ],
      "tags": [
        "new",
        "improve",
    ...
    },
    {
      ...
      "summary": "改善收藏中心信息层级",
      "details": [
        "收藏中心标题提升为 20px，强化面板主标题与内容区域的层级关系",
        "一级标签调整为 14px 常态和选中加重，二级分类降为 12px 并减轻胶囊背景与边框",
        "设计测试页同步收藏标题、一级标签和二级筛选组件，移动端继续保持横向滚动"
      ],
      "tags": [
        "improve",
        "optimize"
    ...
    },
    {
      ...
      "summary": "修正收藏描述实际左对齐",
      "details": [
        "收藏信息列改为明确的左侧起始布局，标题和描述强制占满右侧信息列",
        "提高最终覆盖规则优先级，避免旧版描述居中和移动端隐藏样式重新生效"
      ],
      "tags": [
        "fix"
      ]
    ...
    },
    {
      ...
      "details": [
        "区分首页大搜索框与收藏中心紧凑搜索框，并归纳快捷入口、收藏卡片、顶部 Tab、设置侧栏、表单、按钮、列表和响应式弹窗等场景变体",
        "收藏 favicon 缩小为 26px，移除明显圆形、模糊和投影，仅保留低透明背景与弱描边作为统一边界",
        "收藏卡片明确采用第一行图标与网站名称、第二行描述的层级，让站名和内容先于彩色图标被感知",
        "快捷入口 hover 取消向上位移，避免顶部组件越过容器边界"
      ],
      "tags": [
        "improve"
      ]
